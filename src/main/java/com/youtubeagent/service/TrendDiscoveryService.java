...
public class TrendDiscoveryService {
    ...
    private int discoverFromOpenAi() {
        try {
            ...
            String prompt = """
                    Придумай %d трендовых тем для YouTube Shorts на русском языке.
                    Темы должны быть РАЗНООБРАЗНЫМИ — не повторяй одну и ту же тему!

                    Каждая тема из РАЗНОЙ категории:
                    - Наука и технологии
                    - История и тайны
                    ...
                    Формат ответа — строго JSON:
                    {
                        "topics": [
                            {
                                "topic": "конкретная интересная тема на русском (не общая!)",
                                "category": "категория из списка выше",
                                "reason": "почему это интересно (1 предложение)",
                                "viral_score": 8
                            }
                        ]
                    }
                    ...
                    Правила:
                    - КАЖДАЯ тема должна быть из РАЗНОЙ категории
                    - Темы должны быть КОНКРЕТНЫМИ, не общими (не "факты о космосе", а "Почему на Марсе нет магнитного поля")
                    - Используй актуальные события, открытия, рекорды
                    - Оценивай виральность от 1 до 10
                    """.formatted(config.getTopicsPerRun());
